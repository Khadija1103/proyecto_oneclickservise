...
function procesarPago() {
    ...
    if (!email || email.trim() === "" || !email.includes("@")) {
        Swal.fire({text: "? Debe ingresar un correo electrónico válido", confirmButtonText: "Aceptar"});
        return;
    }

    var telefono = prompt("?? Teléfono del comprador:");
    if (!telefono || telefono.trim() === "" || telefono.replace(/\D/g, '').length < 7) {
        Swal.fire({text: "? Debe ingresar un teléfono válido (mínimo 7 dígitos)", confirmButtonText: "Aceptar"});
        return;
    }
