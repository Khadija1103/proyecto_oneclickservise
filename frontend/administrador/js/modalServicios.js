...
function limpiarImagenesAntiguas() {
    ...
    if (espacio.porcentaje > 75) {
        console.warn('?? Almacenamiento casi lleno. Limpiando imágenes antiguas...');
...
if (!modoCatalogo && formularioAgregarServicio) {
    formularioAgregarServicio.addEventListener("submit", async function(evento) {
        ...
        if (espacio.porcentaje > 80) {
            Swal.fire({text: '?? El almacenamiento está casi lleno. Se limpiarán imágenes antiguas.', confirmButtonText: "Aceptar"});
...
window.cambiarEstadoServicio = function(idServicio) {
    ...
    if (confirm(`?? ¿Desea cambiar el estado del servicio "${servicio.nombre}"?\n\n` +
        ...
        Swal.fire({text: `? Servicio "${servicio.nombre}" ahora está ${servicio.activo ? "Activo" : "Inactivo"}`, confirmButtonText: "Aceptar"});
        renderizarServicios();
    }
};
